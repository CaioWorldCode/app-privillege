...
const Component = () => {
    ...
    const breadcrumbs = [
        { to: '', text: 'Home' },
        { to: 'contracts', text: 'Minhas' },
        { to: 'signatures', text: 'Assinaturas' },
    ]

    const columns = [
        { name: 'Id', acessor: 'id', column: 'id' },
        { name: 'Título', acessor: 'title', column: 'title', format: 'contract_title' },
        { name: 'Descrição', acessor: 'description', column: 'description', format: 'contract_description' },
    ...
    const loadData = async (pageLimit, page) => {
        ...
        try {
            let url = `/api/v1/private/contract_signature/my?order=${orderBy}&limit=${pageLimit}&page=${page}&search=${searchField}`
            ...
            >
                <Modal.Body>
                    ...
                    <Col xs={12} className="mt-4">
                        <span className='text-primary'><b>Título:</b> </span> {dataContent?.contract?.title}
                    </Col>

                    <Col xs={12} className="mt-4">
                        <span className='text-primary'><b>Descrição:</b> </span> {dataContent?.contract?.description}
                    </Col>
                    ...
                    <Col xs={12} className="mt-4">
                        ...
                        <div style={{ margin: 5, padding: 5, border: '1px solid #ccc', whiteSpace: 'nowrap', width: '100%', display: 'inline-block', wordBreak: 'break-all' }}>
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: dataContent?.contract?.content }}
                            />
                        </div>
                    </Col>

                    <Col xs={12} className="mt-4">
                        <span className='text-primary'><b>Assinado em:</b> </span> {dataContent?.created_at}
                    </Col>
                </Modal.Body>
            </Modal>
